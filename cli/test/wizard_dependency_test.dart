...
void main() {
  ...
  group('Wizard Dependency Logic', () {
    test(
      'Disabled state should update based on previous answers in the same run',
      () {
        ...
        fakePrompts.mockedAnswers['Everything looks correct?'] =
            'Confirm and proceed';

        wizard.run();

        final stateB = fakePrompts.capturedStates['Select Feature:']!(
          DependencyFeature.featureB,
        );
        expect(
          stateB?.isDisabled,
          isTrue,
          reason: 'Feature B should be disabled because enable_b is false',
        );
      },
    );
    ...
    test(
      ...
      () {
        ...
        Prompts.instance = _InterceptingFakePromptProvider(fakePrompts, () {
          callCount++;
          if (callCount == 4) {
            // 4th prompt call is 'Select Feature:' in the second loop
